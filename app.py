...
st.markdown("Upload data pasien dan lihat apakah akan kembali dalam 30 hari ke depan berdasarkan model Random Forest (`rf5.pkl`).")

# Load model
@st.cache_resource
def load_model():
    return joblib.load("rf5.pkl")
...
if uploaded_file:
    ...
    fitur = df[['MRN', 'DPJP', 'ADMISSION_DAY', 'ADMISSION_MONTH']].astype(str)
    ...
    hasil = df[df['prediksi_kunjungan'] == 1][['MRN', 'NAMA_PASIEN', 'DPJP', 'ADMISSION_DATE']]
    st.write(hasil)
